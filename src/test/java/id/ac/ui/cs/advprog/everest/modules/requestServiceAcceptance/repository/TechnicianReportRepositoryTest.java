...
//        assertTrue(reportsForRequest1.stream().allMatch(report -> report.getUserRequest().getRequestId().equals(requestId1)));
    }
    ...
    private TechnicianReport createReport(UUID technicianId, UserRequest userRequest, String status) {
        ...
        switch(status) {
            ...
            case "COMPLETED":
                ...
                report.complete();
                break;
            // "DRAFT" is the default state, so no action needed
        }
        return report;
    }
}